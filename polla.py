...
class Polla:
    ...
    def load_predictions(self):
        for file in glob(self.predictions_files):
            ...
            if "./predictions/" in participant_name:
                participant_name = participant_name.split("./predictions/")[-1]
    ...
    def update_goals(self):
        ...
        for result in latest_scores:
            ...
            index_match = self.master_plan[
                (self.master_plan.Status != "jugado")
                & (self.master_plan.Local == local_team)
                & (polla.master_plan.Visita == visit_team)
            ].index
    ...
    def compute_results(self):
        self.master_plan["results"] = self.master_plan.apply(
            lambda row: self._get_tendencia(row), axis=1
        )
    ...
    def build_scoreboard(self):
        scoreboard = {
            k: self._get_points(k, self.master_plan)
            + self._get_points_finalists(k, self.finalists)
        ...
        self.scoreboard = pd.DataFrame(
            {
                "Position": [x for x in range(1, len(self.participants) + 1)],
                "Participante": scoreboard.keys(),
                "Points": scoreboard.values(),
            }
        ).set_index("Position")
